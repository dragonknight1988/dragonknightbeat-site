...
if [ "$MEM_AVAIL" -lt 300 ] || [ "$SWAP_PCT" -gt 40 ]; then
  # 🐕 保护：内容生成任务运行中则跳过本轮重启，避免杀掉生成进程
  if pgrep -f "gen-cron.sh|generate_[a-z_]+\.py" >/dev/null 2>&1; then
    echo "$(date "+%Y-%m-%d %H:%M") ⏭️ 内存告警(${MEM_AVAIL}MB)但检测到内容生成任务运行中，跳过本轮重启" >> "$LOG"
    exit 0
  fi
